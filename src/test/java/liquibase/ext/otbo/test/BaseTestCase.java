package liquibase.ext.otbo.test;
...
public class BaseTestCase {
	...
	public static void connectToDB() throws Exception {
		if ( connection == null ) {
			...
			driver = (Driver) Class.forName( DatabaseFactory.getInstance().findDefaultDriver( url ), true, Thread.currentThread().getContextClassLoader() ).getConstructor().newInstance();
	...
	public static void update() {
		...
		map.put( "database", liquiBase.getDatabase() );
		map.put( "resourceAccessor", liquiBase.getResourceAccessor() );

		try {
			Scope.child( map, () -> {
				CommandScope cmd = new CommandScope( "update" );
				cmd.addArgumentValue( "database", liquiBase.getDatabase() );
				cmd.addArgumentValue( "changelogFile", liquiBase.getChangeLogFile() );
				cmd.execute();
			} );
		} catch ( Exception e ) {
